...
const EditBooking = () => {
        ...
        useEffect(() => {
            const fetchRoomNums = async () => {
                try {
                    ...
                    console.error("Error fetching rooms number:", error);
                }
            };
            fetchRoomNums();
        }, [token]);
    ...
    const handleChange = (event) => {
        ...
        name === 'user_id'?setUserId(event.target.value):'';
        name === 'hotel_id'?setHotelId(event.target.value):'';
        name === 'room_id'?setRoomId(event.target.value):'';
        name === 'room_number'?setRoomNumber(event.target.value):'';
        name === 'check_in'?setCheckIn(event.target.value):'';
        name === 'check_out'?setCheckOut(event.target.value):'';
        name === 'total_price'?setTotalPrice(event.target.value):'';
        name === 'payment_method'?setPaymentMethod(event.target.value):'';
        name === 'status'?setStatus(event.target.value):'';
    }
    const getUser = async () => {
            ...
            <div className="col-md-5">
                <div className="card" style={{ marginTop:'10px'}}>
                    ...
                    <form onSubmit={handleUpdate}>
                        ...
                        <Link to="/admin/booking" style={{marginLeft:'18%'}} className="btn btn-primary float-start">Lihat Data</Link>
